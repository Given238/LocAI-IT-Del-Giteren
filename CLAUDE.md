...
> **Status: Phases 1-6 complete and verified** (ETL, backend, frontend, geospatial distance + locale-tone, chat interface over the existing `/itinerary` logic, voice input/output, interactive preference onboarding). Project has since expanded scope per mentor direction: chatbot-first interface, onboarding flow, auth, and a knowledge-retrieval layer. The original form-based itinerary tool is being kept as a working fallback throughout — nothing gets removed as new layers are added.
>
...
The locale value (SEA origin, or "Others") is used by THREE features: narrative tone (Phase 4), voice output (Phase 5b), and the onboarding flow (Phase 6). These are NOT three separate, potentially-conflicting inputs -- as of Phase 6, the actual data flow is:

- `App.jsx` seeds its shared `locale` state from the stored profile (`loadProfile()?.locale || ""`) once, at load. This is the ONE piece of React state read by `ItineraryForm`'s "Narrative tone" field, `ChatView`'s locale dropdown (which is also what voice output reads), and now `Onboarding`.
- The standalone dropdown (Phase 4, still visible in both Form and Chat) is a live per-session override on top of that seed -- selecting a different locale there updates the shared state immediately (so narrative tone/voice reflect it right away) but does **not** write back to the stored profile. Verified: temporarily switching locale via the dropdown, then reloading the page, reverts to the profile's saved value, not the temporary one -- exactly the "try a different locale right now without changing your saved preference" behavior this was meant to have.
- Completing or editing onboarding (the "⚙ Edit my preferences" button) IS a deliberate, explicit profile update -- finishing it calls `saveProfile()` and immediately re-seeds the shared `locale` state from the new value, so it takes effect right away without needing a reload.
- Before onboarding has ever run (or a guest who skipped it), there is no profile locale, so the seed is `""` (neutral) and the standalone dropdown behaves exactly as it did before Phase 6 existed -- no special-casing needed, this falls out of the seed logic naturally.
...
4. **Login/signup + Google OAuth** — the real next item; migrates `profile.js`'s local storage to a per-account backend once it exists (flagged above)
