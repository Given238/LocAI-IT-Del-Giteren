...
| **Interactive map (Leaflet)**            | ✅ Done                                  | `frontend/src/components/MapView.jsx`; react-leaflet + OSM tiles, no API key; click a place card to draw an approximate straight-line distance from the start hub; single persistent map instance (verified via DOM identity check, not remounted per click); hidden entirely when start_location is unrecognized |
