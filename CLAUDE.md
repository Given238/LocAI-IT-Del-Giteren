...
> **Status: Phases 1-7 complete and verified** (ETL, backend, frontend, geospatial distance + locale-tone, chat interface over the existing `/itinerary` logic, voice input/output, interactive preference onboarding, email/password + Google auth). Project has since expanded scope per mentor direction: chatbot-first interface, onboarding flow, auth, and a knowledge-retrieval layer. The original form-based itinerary tool is being kept as a working fallback throughout — nothing gets removed as new layers are added.
>
...
| **Interactive preference onboarding (Phase 6)**  | ✅ Done                            | `frontend/src/components/Onboarding.jsx`, `profile.js`, `validation.js`. Step-by-step (location → budget → duration → interests → locale/origin incl. new "Others" option → review), reusing the exact same field names/validators as `ItineraryForm` -- extracted `validation.js` so there's one set of rules, not two. Originally shipped with no auth (browser-local `localStorage` only) -- **now migrated**, see the Phase 7 row below: logged-in users get a real backend-stored profile, guests keep the exact `localStorage` behavior described here unchanged, and a guest's local profile auto-uploads on first login/signup rather than being lost. Skippable at the welcome step or via the always-present ✕, and editable anytime via a new "⚙ Edit my preferences" header button (re-opens pre-filled; ✕ during an edit discards changes instead of re-saving, verified distinctly from first-run skip). Locale precedence now fully verified end-to-end: profile locale seeds the shared `locale` state on load; the standalone dropdown remains a live per-session override that never writes back to the stored profile (verified: temporarily switching locales then reloading reverts to the saved profile value, not the temporary one). Form and chat both pre-fill every field from the profile (form via a `key`-remount on profile save; chat via a "Use my saved trip" quick-start chip that composes and sends a normal chat message through the *exact* same `/chat` path -- no parallel data model, no new endpoint). **Two real bugs found while testing the quick-start chip, not hypothetical:** (1) the composed message used `toLocaleString("id-ID")` for the budget (e.g. "Rp500.000", period as thousands separator) -- fine for display, but sent as the literal chat message it confused the LLM's own number parsing into asking a clarifying question instead of calling the tool; fixed by keeping the formatted string only in the button's visible label and sending the raw number in the actual message. (2) More fundamentally, in `backend/chat.py`: when only one interest is mentioned in natural phrasing ("interested in nature", no plural cue), the model emits `interests="nature"` (a bare string) instead of `interests=["nature"]`, which `ItineraryRequest`'s `list[str]` field correctly rejects -- this doesn't just affect the chip, it would hit *any* real chat user mentioning exactly one interest. Fixed at the source: `handle_chat` normalizes a bare-string `interests` into a single-item list before constructing `ItineraryRequest`. Re-ran `scripts/test_chat.py` after the fix to confirm zero regression on the existing missing-info/clarify/PDF flow. |
| **Login/signup + "Continue with Google" (Phase 7)** | ✅ Done                         | `backend/auth.py`, `auth_schema.py`; `users` / `user_profiles` / `sessions` tables (idempotent `CREATE TABLE IF NOT EXISTS`, separate from `scripts/etl.py`'s destructive DDL so re-running the dataset ETL never touches accounts). **Session design: server-side session + `HttpOnly` cookie, not JWT** -- decided and explained before implementing, per instruction. JWT's main benefit (stateless scaling) doesn't matter for a single-instance hackathon demo, and its real downside does: revocation needs a blacklist (defeats statelessness anyway), and the common JWT-in-`localStorage` pattern is exactly the client-side token exposure NFR10 warns against. A DB-backed opaque token (`secrets.token_urlsafe(32)`) never touches JS-reachable storage; logout is just deleting a row. **Guest access: explicitly kept, recommended and confirmed working** -- chat/voice/form/onboarding all still work with zero login (verified as a regression check); auth is additive (cross-device persistence + personalization), never a gate. `/auth/profile` (GET via `/auth/me`, PUT to save) is the one thing that genuinely requires login, since there's nothing to read/write server-side for a guest. **Google OAuth implemented but not live-testable:** `.env` has no real `GOOGLE_OAUTH_CLIENT_ID/SECRET` (none exist yet), so the live Google handshake can't be exercised end-to-end. Server-side authorization-code exchange is implemented per spec (code → token → userinfo, client secret never sent to the frontend) and `GET /auth/config` reports `google_configured: false`, which the frontend's "Continue with Google" button checks to hide itself rather than present a broken link -- verified this actually hides in a real browser with no credentials configured. **Frontend profile source now three-tiered:** logged-in → backend `user_profiles` row (via `/auth/me` + `PUT /auth/profile`); guest → `localStorage` (Phase 6, unchanged); **first login/signup with an existing guest profile → the local profile is auto-uploaded** to the new account (verified: onboard as guest, then sign up, and the guest answers appear in the account with onboarding not re-triggered). Locale precedence extends the same shared `App.jsx` state -- backend profile's locale wins over local when logged in, same "profile first, dropdown is a session-only override" rule as before. |
...
The locale value (SEA origin, or "Others") is used by narrative tone (Phase 4), voice output (Phase 5b), and the onboarding flow (Phase 6/7). These are NOT separate, potentially-conflicting inputs -- as of Phase 7, the actual data flow is:

- `App.jsx` seeds its shared `locale` state from whichever profile is authoritative once, at load: a logged-in user's backend `user_profiles` row if a session exists, else the `localStorage` profile, else `""` (neutral). This is the ONE piece of React state read by `ItineraryForm`'s "Narrative tone" field, `ChatView`'s locale dropdown (which is also what voice output reads), and `Onboarding`.
- The standalone dropdown (Phase 4, still visible in both Form and Chat) is a live per-session override on top of that seed -- selecting a different locale there updates the shared state immediately (so narrative tone/voice reflect it right away) but does **not** write back to the stored profile, backend or local. Verified: temporarily switching locale via the dropdown, then reloading the page, reverts to the profile's saved value, not the temporary one -- exactly the "try a different locale right now without changing your saved preference" behavior this was meant to have.
- Completing or editing onboarding (the "⚙ Edit my preferences" button) IS a deliberate, explicit profile update -- finishing it saves to the backend if logged in (`PUT /auth/profile`) or `localStorage` if a guest, and immediately re-seeds the shared `locale` state from the new value either way, so it takes effect right away without needing a reload.
- Logging in migrates a guest's local profile to the backend automatically if the account has none yet (see Phase 7 row above); logging out reverts to whatever local/guest profile exists, which may be none.
- Before any profile exists at all (fresh guest, or freshly signed-up account with nothing saved), the seed is `""` (neutral) and the standalone dropdown behaves exactly as it did before Phase 6 existed -- no special-casing needed, this falls out of the seed logic naturally.
...
| Auth               | Google OAuth 2.0 (server-side code exchange) + email/password (bcrypt)                             | Session = server-side `sessions` table + `HttpOnly` cookie, not JWT (see Phase 7 row above for why). Guest use fully allowed everywhere. |
...
GOOGLE_OAUTH_CLIENT_ID=...        # optional -- "Continue with Google" hides itself if unset
GOOGLE_OAUTH_CLIENT_SECRET=...
GOOGLE_OAUTH_REDIRECT_URI=http://127.0.0.1:8000/auth/google/callback
FRONTEND_URL=http://localhost:5173
SESSION_SECRET=...                # reserved for future cookie-signing/CSRF hardening, not required now
COOKIE_SECURE=false               # set true in production (HTTPS)
COOKIE_SAMESITE=lax               # only "none" if frontend/backend end up on genuinely different sites
```

Never commit these. `.env` is gitignored; `.env.example` holds the template only. **As of this update, `.env` has no real `GOOGLE_OAUTH_CLIENT_ID/SECRET`** -- Google login is implemented but untested end-to-end; create a client at console.cloud.google.com/apis/credentials to actually exercise it.
...
3. ~~**Interactive preference-onboarding flow**~~ ✅ Done — step-by-step, reuses the same fields/validation as the form, stored as a local profile (see Build Progress row above); locale is now genuinely the single source of truth described below, not just a plan for it
4. ~~**Login/signup + Google OAuth**~~ ✅ Done — email/password + Google OAuth (implemented, untestable end-to-end without real Google credentials), server-side session cookie, guest access fully preserved; `profile.js`'s local storage now migrates to the backend on first login (see Build Progress row above)
