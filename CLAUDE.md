...
> **Status: Phases 1-4 complete and verified, chatbot orchestrator now also done** (ETL, backend, frontend, geospatial distance + locale-tone, chat interface over the existing `/itinerary` logic). Project has since expanded scope per mentor direction: chatbot-first interface, onboarding flow, auth, and a knowledge-retrieval layer. The original form-based itinerary tool is being kept as a working fallback throughout — nothing gets removed as new layers are added.
>
...
| **Voice input/output (Phase 5b)**                | ⏳ Next                            | Browser Web Speech API (STT + TTS), voice selected from existing locale value — no new backend dependency                                                                                               |
...
- **Before onboarding exists (now through Phase 5b):** the standalone locale dropdown built in Phase 4 is the only source. Keep it working as-is.
...
2. **Voice input/output (Phase 5b)** — browser Web Speech API on top of the chatbot; voice driven by the existing locale value
